...
const createToken = (id, email, role) => { return jwt.sign({ id, email, role }, process.env.JWT_SECRET, { expiresIn: "30d" }) }
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    const token = await createToken(user._id, user.email, user.role);
